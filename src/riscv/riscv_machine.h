 ...
 * - Memória de 32-bit word-addressable (ou byte-addressable conforme configuração)
 * - Instruções de 32 bits fixas (sem suporte a compressed/C-extension neste momento)
 ...
 * - Override dos métodos virtuais principais (decodeInstruction, executeInstruction)
...
public:
     ...
     */
    const DecodedInstruction& getCurrentInstruction() const { return currentInstr_; }
    
    /**
     * @brief Verifica se há um hazard de controle pendente
     * 
     * Usado para detecção básica de hazards em implementação single-cycle.
     * Em single-cycle puro, não há hazards, mas deixamos hook para futuro pipeline.
     */
